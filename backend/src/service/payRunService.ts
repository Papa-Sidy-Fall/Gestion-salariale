...
export class PayRunService {
  ...
  static async getAllPayRuns(companyId?: string) {
  ...
  static async updatePayRun(id: string, data: any) {
  ...
  static async approvePayRun(id: string) {
    const payRun = await prisma.payRun.findUnique({
      where: { id },
      include: {
        payslips: true,
  ...
  static async closePayRun(id: string) {
    const payRun = await prisma.payRun.findUnique({
      where: { id },
      include: {
        payslips: true
    ...
    const unpaidPayslips = payRun.payslips.filter((p: any) => p.status === 'EN_ATTENTE');
  ...
  static async getPayRunsByCompany(companyId: string) {
    const payRuns = await prisma.payRun.findMany({
      where: { companyId },
      include: {
        payslips: {
